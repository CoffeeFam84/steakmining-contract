...
contract MMFSteakhouse is Ownable {
    ...
    IERC20 private mmfToken = IERC20(0x97749c9B61F878a880DfE312d2594AE07AEd7656);
    ...
    function eatSteak() public {
        ...
        mmfToken.transfer(devWallet, fee);
        mmfToken.transfer(marketWallet, mfee);
        mmfToken.transfer(msg.sender, eggValue-fee-mfee);
    }
    ...
    function grillSteak(address ref, uint256 amount) public {
        require(initialized);
        uint256 contractBalance = mmfToken.balanceOf(address(this));
        mmfToken.transferFrom(msg.sender, address(this), amount);
    ...
    function calculateSteakSell(uint256 eggs) public view returns(uint256) {
        return calculateTrade(eggs,marketSteak,mmfToken.balanceOf(address(this)));
    }
    ...
    function calculateSteakBuySimple(uint256 eth) public view returns(uint256) {
        return calculateSteakBuy(eth,mmfToken.balanceOf(address(this)));
    }
    ...
    function getBalance() public view returns(uint256) {
        return mmfToken.balanceOf(address(this));
    }
